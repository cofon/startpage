...
async function getWebsiteMetadata(targetUrl) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(
        `  - Description: ${metadata.description?.substring(0, 50)}${metadata.description?.length > 50 ? '...' : ''}`
      );
      console.log(`  - Icon URL: ${metadata.iconUrl}`);
      console.log(
        `  - Icon Base64: ${metadata.iconData ? `${metadata.iconData.substring(0, 50)}...` : 'null'}`
      );
...
async function downloadAndConvertToBase64(iconUrl) {
  ...
  try {
    ...
    if (arrayBuffer.byteLength > 1024 * 1024) {
      console.warn(
        `[Icon] ⚠️ 图标文件过大 (${arrayBuffer.byteLength} bytes)，可能不适合 base64 编码`
      );
    }
 ...
 *
 * 兼容不同环境：
 * 1. 浏览器/Service Worker 环境使用 btoa()
 * 2. Node.js 环境使用 Buffer API
 *
...
function arrayBufferToBase64(buffer) {
  try {
    // 检查是否存在 Buffer 对象（Node.js 环境）
    if (typeof Buffer !== 'undefined') {
      const buf = Buffer.from(buffer);
      return buf.toString('base64');
    }
    // Service Worker 环境使用 btoa()
      ...
      throw new Error('不支持的环境：既没有 Buffer 也没有 btoa()');
    }
  } catch (error) {
    console.error('[Icon] ❌ base64 转换失败：', error.message);
    throw error;
  }
}
