...
- **条件组**：条件可以分组，每个组内的条件是 OR 关系，组间是 AND 关系
- **添加条件**：点击"+ 添加条件（OR）"在当前组内添加条件
- **添加条件组**：点击"+ 添加条件组（AND）"添加新的条件组
- **删除条件**：点击条件旁边的 × 按钮删除单个条件
- **删除条件组**：点击条件组旁边的 × 按钮删除整个条件组
- **重置条件**：点击"重置条件"按钮将条件重置为默认状态
...
| 不等于 | 字段值不等于指定值 | 排除特定值 |
| 包含 | 字段值包含指定值 | 模糊搜索 |
| 不包含 | 字段值不包含指定值 | 排除特定内容 |
| 为空 | 字段值为空 | 查找空值 |
| 不为空 | 字段值不为空 | 查找非空值 |
| 有效值 | 字段值有效 | 对于图标数据，检查是否是有效的 Base64 编码 |
| 无效值 | 字段值无效 | 对于图标数据，检查是否是无效的 Base64 编码 |
| 大于 | 字段值大于指定值 | 数值比较 |
| 小于 | 字段值小于指定值 | 数值比较 |

##### 导出选项

- **导出格式**：支持 JSON 和 CSV 格式
- **导出字段**：可选择要导出的字段
- **排序选项**：可按指定字段排序
- **包含其他数据**：可选择是否包含设置、主题和搜索引擎数据

##### 导入数据

输入 `--import` 打开导入面板，上传 JSON 文件恢复数据。
