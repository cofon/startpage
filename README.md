...
- **显示标签列表**：搜索框获得焦点、双击搜索框或按下Enter键时显示标签列表
...
输入 `-layout` 打开布局模式选择面板，支持以下布局模式：

- **极简模式**：一行显示图标、标题/描述/名称和操作按钮，布局紧凑
- **默认模式**：标准列表布局，显示图标、标题、描述和操作按钮
- **完整模式**：详细信息布局，显示所有字段，包括ID、URL、标签、创建时间、更新时间和访问次数
