...
## 未完成工作
新的需求：
1. 创建一个浏览器插件，win11, edge；
2. 插件的功能：
    - 插件维护一个indexeddb，存储用户添加的网站；
    - 插件提供一个快捷键：alt+shift+d,按下快捷键后，会弹出一个UI
    - 快捷键弹出的UI有输入框，填充网站的title desc icon等数据
    - 快捷键弹出的UI有保存按钮，点击按钮后，会保存用户输入的数据到indexeddb中
    - 可以导入数据
3. 当前项目(浏览器起始页)可以 增删改查 插件维护的数据
