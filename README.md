...
## 未完成工作
修改后的方案：
1. 起始页 --add 打开添加单个网站页面，需要输入URL，发送信息给插件，插件返回title desc icon数据(已经实现);
2. 起始页 --import 打开导入网站页面，导入json, 如果数据不完整，补全数据，title desc icon需要发送消息给插件，让插件返回数据(还没有实现);
3. 起始页 --export 打开导出网站页面，导出完整的json数据，包括websites themes searchEngines settings(已经实现);
4. 插件添加网站页面 可以添加单个网站, 可以直接添加当前浏览器打开的页面，不需要输入URL，自动获取URL，自动补全数据(已经实现);
5. 插件导入网站页面 可以导入json，如果数据不完整，补全数据(删除，因为起始页已经有了);
6. 插件导出网站页面 可以导出网站，只有websites，不包含themes searchEngines settings(删除，因为起始页已经有了);
7. 起始页 --adds 打开批量添加网站页面，(合并到2, 判断json是否有websites urls，websites是单前的逻辑，urls是批量添加逻辑);
8. --补全，点击按钮自动遍历数据库，数据不完整的网站发送给插件，插件补全字段返回数据，更新数据库(还没有实现);

- 起始页导入数据完成，细节未处理

显示 marked list 的时候，如果marked list没有数据，
搜索模块的输入框获取到焦点会显示tags，会显示滚动条，
因为搜索模块占用的高度比较小，tags是显示在上层的，不占用高度，
优化搜索模块的高度占用，放置marked-list空时input获取焦点出现滚动条的情况
...
{"websites":[{website1},{website2}], "themes":[{theme1},{theme2}], "searchEngines":[{searchEngine1},{searchEngine2}], "settings":{}}
