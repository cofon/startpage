...
#### 导入主题
- 使用 `-import` 命令导入包含主题数据的 JSON 文件
- 支持同时导入多个主题

### 批量修改

输入 `-batch` 打开批量修改面板，支持以下功能：

#### 步骤 1：筛选条件
- 设置条件来筛选需要修改的网站
- 支持多个条件组，组内条件是 OR 关系，组间条件是 AND 关系
- 支持多种操作符：等于、不等于、包含、不包含、为空、不为空、有效、无效、大于、小于

#### 步骤 2：选择操作
- **字段修改**：修改网站名称、标题、URL、描述等字段
- **标签操作**：添加、移除、替换标签
- **图标操作**：重新获取图标、设置默认图标、清除图标
- **状态操作**：修改标记状态、激活状态、隐藏状态
...
**使用示例**：
```
-active & -title 知乎  # 显示活跃且标题包含"知乎"的网站
-baidu | -google      # 显示包含"baidu"或"google"的网站
-active -tag 工作 & -title 教程  # 显示活跃且包含"工作"标签且标题包含"教程"的网站
-active -tag 工作 | -title 教程  # 显示活跃且包含"工作"标签或标题包含"教程"的网站
```
...
搜索框输入内容是 - 的时候 ，显示可输入命令列表，
输入框输入 - ，显示所有可输入命令列表，但是 hidden unhidden unactive 不在列表内
输入框输入 -a , 显示a开头的命令列表，比如：active add 
输入框输入 -ac , 显示ac开头的命令列表，比如 active
命令名字输入完成，命令列表隐藏；
命令名字输入完成的判断标准是，命令列表中有输入的命令名字；
比如，用户输入 -active ，命令列表中就有 active 命令，此时应该隐藏命令列表；

用户输入第二个命令的时候，再次显示命令列表
输入第二个命令名字完成，命令列表隐藏；

只要用户要输入命令，就会显示命令列表，
用户输入命令名字完成，命令列表隐藏；
用户按 esc 不管命令名字是不是输入完成，都会隐藏命令列表；

用户要输入命令的判断标准是 输入框输入 - ；
不管是用户输入的第几个命令，看到 - 就显示命令列表；
...
用户输入 -active, 命令列表隐藏；
用户输入 -active - ， 显示命令列表；
用户输入 -active -title， 命令列表隐藏；

tag的提示：
用户输入 -tag, 显示tag列表；
用户输入 -active -tag, 显示tags列表；
tag标签输入完成，tag列表隐藏；
tag标签输入完成的标准是，-tag xxx，xxx在tags列表中；


用户按 esc 隐藏tag列表；
用户输入 -xxx空格，即便xxx不是命令名，只要xxx后有空格，就隐藏命令列表
