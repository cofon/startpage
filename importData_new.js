      ...
      function processSettings() {
        return new Promise((resolve, reject) => {
          ...
          if (data.settings.id && data.settings.id !== 'settings') {
            console.warn('跳过无效设置：id字段必须为settings', data.settings)
            resolve()
            return
          }
          ...
          // 验证selectedThemeId是否存在于themes表
          const themeReq = themesStore.get(settingsToImport.selectedThemeId)
          themeReq.onsuccess = () => {
            if (!themeReq.result && data.settings.selectedThemeId) {
              // 如果主题不存在，保持原值不修改
              settingsToImport.selectedThemeId = data.settings.selectedThemeId
            }

            // 验证selectedSearchEngineId是否存在于searchEngines表
            const engineReq = searchEnginesStore.get(settingsToImport.selectedSearchEngineId)
            engineReq.onsuccess = () => {
              if (!engineReq.result && data.settings.selectedSearchEngineId) {
                // 如果搜索引擎不存在，保持原值不修改
                settingsToImport.selectedSearchEngineId = data.settings.selectedSearchEngineId
              }

              // 更新settings
              const putReq = settingsStore.put(settingsToImport)
              putReq.onsuccess = () => resolve()
              putReq.onerror = () => {
                console.error('更新设置失败:', settingsToImport)
                reject(new Error('更新设置失败'))
              }
            }
            engineReq.onerror = () => {
              console.error('检查搜索引擎ID失败:', settingsToImport.selectedSearchEngineId)
              reject(new Error('检查搜索引擎ID失败'))
            }
          }
          themeReq.onerror = () => {
            console.error('检查主题ID失败:', settingsToImport.selectedThemeId)
            reject(new Error('检查主题ID失败'))
          }
        })
      }
